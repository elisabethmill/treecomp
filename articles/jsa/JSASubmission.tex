...
Figure \ref{fig:predicted-vs-actuals} shows actual vs. predicted QBR for all quarterbacks in the dataset. Recall that \textcolor{red}{77\%} of actual QBRs are zero, which the visualization does not make clear. The errors are very right-skewed, and this further shows why so few college quarterbacks exceed a QBR prediction of 40. 

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{predicted_vs_actuals.pdf}
  \caption{\it Predicted and actual (regressed) QBR for all quarterbacks in the dataset. The vast majority \textcolor{red}{(77\%)} of actual QBRs are zero, meaning that the points along the x-axis represent \textcolor{red}{77\%} of the data.}
  \label{fig:predicted-vs-actuals}
\end{figure}

\subsection{Case Study: 2025 NFL Draft}

Table \ref{tab:top-ten} shows the QBR predictions for the top 10 predicted quarterbacks in the 2025 draft. \textcolor{red}{Using both the classification and regression models, predictions are calculated as  
\begin{equation}
...
While the predictions are interesting by themselves, the focus of the present work is interpreting how we arrive at them. Table \ref{tab:side-by-side-similarity} reports the ten most similar historical draft prospects for \textcolor{red}{four of the top quarterback} prospects: \textcolor{red}{Cam Ward, Shedeur Sanders, Jaxson Dart, and Tyler Shough. These similarity scores result from the regression model, so the comps are players that have/had a QBR value above zero.} 
Recall that the similarity scores reported in this table are interpretable as the weight of each historical prospect in the predictions in Table \ref{tab:top-ten}, \textcolor{red}{which combine both the estimated probability of a positive QBR (classification model) and the conditional expected QBR given a positive outcome (regression model).} The highest similarity score for any of these prospects is only \textcolor{red}{3.7\%}. This is a helpful reminder that quarterback career outcomes are noisy and speaks to the fallacy of putting significant stock in a single player comparison. The random forest model learns to spread out the weight across historical players, rather than concentrating a lot of weight in the single most similar player. With access to more granular athleticism data, the weight might concentrate more in fewer comps.

\textcolor{red}{For each of the prospects, we are able to calculate their effective sample size (ESS) to capture the number of training players from a random sample to obtain a prediction \citep{mccaffrey_propensity_2004}, with the formula: $$\text{ESS}=\frac{\sum_{i=1}^n(w_{i})^2}{\sum_{i=1}^{n}w_i^2}.$$}

\textcolor{red}{Additionally, modifying the model's hyperparameters can result in sparser and more concentrated weights. Figure \ref{fig:ESS-vs-hyperparameter-values} is a box plot showing the relationship between the effective sample size and three random forest hyperparameters. We can see that reducing the minimum node size results in sparser weights, while reducing max depth results in more concentrated weights. The relationship with mtry is not as strong as the other two hyperparameters, but an mtry value of 1 appears to result in more concentrated weights. However, as mtry increases past 1, there is a plateau.
}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ESS_vs_hyperparameter_values.pdf}
    \caption{\it \textcolor{red}{Visualization of the relationship between effective sample size and the max depth, min node size, and mtry hyperparameters in both the classification and regression models. In both models, lower max depth values were associated with higher ESS values, and lower min node size values were associated with lower ESS values.
    }}
    \label{fig:ESS-vs-hyperparameter-values}
\end{figure}

\textcolor{red}{Table \ref{tab:prospect-ess} shows the effective sample size calculations for each of the four chosen prospects, as well as the number of comps needed for 50\%, 75\%, and 90\% of the prediction weight. It also shows what total percentage of the final prediction the comps in table \ref{tab:side-by-side-similarity} are responsible for.}

\begin{table}[H]{
    \input{prospect_ess.tex}
  }
  \caption{\it \textcolor{red}{The effective sample size, number of comps needed for 50\%, 75\%, and 90\% of the final prediction, and what weight of the prediction the top ten comps represent.}} % maybe improve this caption
  \label{tab:prospect-ess}
\end{table}
...
It is worth noting that both Ward and Sanders started their college careers at teams that are not in the NCAA Division I FBS, as Ward played his first season at Incarnate Word and Sanders played his first two seasons at Jackson State. As a result, their college statistics used in the model do not include their statistics from these schools. \textcolor{red}{We observe that Gabriel has a slightly higher QBR prediction than both Ward and Sanders although both were considered better prospects.} However, Gabriel finished higher than \textcolor{red}{both in Heisman voting, and Oregon had a much higher strength of schedule than both Colorado and Miami.}
Dart's relatively low prediction compared to the \textcolor{red}{top} three makes sense because he wasn't considered a top quarterback prospect until his performance in the NFL Combine, which this model does not take into consideration. He also performed well in his interviews with teams \citep{traina_qb_2025}, which is not quantifiable. \textcolor{red}{Tyler Shough's prediction is 3.5, which is significantly lower than the other three prospects. This makes sense because he was not considered to be a consensus top quarterback prospect. However, his draft stock rose a bit after the Reese's Senior Bowl and NFL Draft Combine. His primary strengths were his maturity, work ethic, athleticism, and stature, which the model does not take into consideration \cite{mcgavic_tyler_2025}. Additionally, his per season stats are likely lower than the others because he only completed one full season of production out of his seven total years of college football due to injuries.}

Figure \ref{fig:prospect-plots} provides a full breakdown of the random forest predictions based on each prospect's similarity scores for all of the historical prospects in the training data. Each prospect's prediction is a weighted average of past prospects' outcomes, and the weighted histograms in Figure \ref{fig:prospect-plots} visualize that weighted mean. Ward has the highest concentration of comps between 55 and 65 QBR, which is why his prediction is the highest \textcolor{red}{of these four prospects}. On the opposite end of the spectrum, 
\textcolor{red}{Shough} has the highest concentration of training players near zero QBR, explaining why his prediction was by far the lowest of the four prospects.
