...
All data used for this study are publicly available from Sports Reference \citep{sports_reference_sports_2025}. Our dataset includes \revision803} quarterbacks who (1) played their final season at the NCAA Division I Football Bowl Subdivision (FBS) between 2001 and \revision{2022 (inclusive), because of the 188 quarterbacks who had started in the NFL, 145 started within 2 seasons of being drafted}; and (3) attempted more than \revision{500 passes during their career}. The data for each player-season include team, conference, strength of schedule, games played, passing statistics (attempts, completions, touchdowns, interceptions), rushing statistics (attempts, yards, touchdowns), and awards (All-America designation, Heisman voting).
...
Average QBR per season is the response variable describing the success of quarterback prospects. In the dataset, \revision{77\%} of college quarterbacks achieve zero QBR in the NFL, so the distribution is very right-skewed. \revision{This percentage makes sense because let's say that each of the 136 FBS schools graduates one QB prospect every three years, resulting in 78 eligible prospects each year. 23\% of these prospects translates to about 10 quarterbacks per year who will play in an NFL game at some point in their career (and not necessarily as a starter). Normally, the number of NFL quarterback vacancies hovers around 7 each year, leaving three players who will play at some point as backups.}
...
\end{equation*}
We chose $c = 20.7$ as this value minimizes the error in predicting a player's next-season QBR when used to regress current-season QBR. \revision{The shrinkage parameter $c$ controls how strongly small-sample QBR values are pulled toward zero. We estimate $c$ using historical, year-to-year QBR data by choosing the value that minimizes the squared error between observed next-season QBR and the mean-regressed prediction. Here, $i$ indexes quarterbacks and $t$ indexes seasons:}
...
We trained the random forest model using the R package ranger \citep{wright_ranger_2024}, which is a fast implementation of the random forest algorithm. \revision{In order to account for the large number of quarterbacks that had a QBR value of zero, we used two separate random forest models with the same predictor variables. The college statistics used as predictor variables were games played per season, \revision{attempted} passes per season, complete passes per season, passing yards per season, passing yards per attempt, passing touchdowns per season, passing interceptions per season, rushing attempts per season, rushing yards per season, rushing touchdowns per season, number of All-America selections, final-season \revision{Heisman} Award voting finish, final-season strength of schedule, final-season games played, final-season passing yards per attempt, and final-season passer efficiency rating. The first model is a classification model where the response variable is whether or not the player will have a QBR value that is not zero, and this model was trained on all of the data. The second model's response variable is the average QBR across NFL seasons, and the training data excluded any player with a QBR value of zero. This two-model approach is analogous to a two-part model commonly used for zero-inflated outcomes, where one model predicts the probability of a nonzero response and a second model predicts the magnitude conditional on being nonzero.} 

To train the random forest model, \revision{we designated data from the 2006, 2007, 2008, 2010, 2011, 2012, 2014, 2015, 2016, 2018, 2019, 2020, and 2022 seasons as training data and the data from the 2009, 2013, 2017, and 2021 seasons as testing data.} We chose $B$ = 1,000 decision trees, each trained on a \revision{bootstrap} sample of the data, with replacement. To determine the number of variables to consider at each node and the maximum tree size, we used \revision{temporal cross-validation on both the classification and regression models to determine the hyperparameters that minimize the RMSE. Table \ref{tab:hyperparameters} shows the hyperparameters for the training dataset and the full dataset.}
...
Figure \ref{fig:importance} shows the importance of each feature learned by the random forest \revision{models}. \revision{The models explained 27.7\% of the variance of regressed QBR in the test set. A linear regression model with the same covariates only explained 22.62\% of the variance of regressed QBR in the test set.} \revision{For the first classification model, final-season strength of schedule, final-season passer rating, and average passing performance (yards, touchdowns) stand out as the most important predictors of having a QBR value above 0. For the second regression model, average passing performance (attempts, completions, yards), average rushing performance (yards, touchdowns, attempts), and final-season passer rating are the top predictors of NFL success.} Note that yards per season, touchdowns per season, and completions per season are highly correlated with each other---each pairwise correlation is at least 0.94.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{cls_variable_importance.pdf}
    \includegraphics[width=0.7\linewidth]{rgr_variable_importance.pdf}
    \caption{\it Importance of predictor variables in \revision{both the classification and regression random forest models. The response variable in the classification model is whether the player will have a regressed NFL QBR value above zero. The response variable in the regression model is regressed NFL QBR, averaged across seasons in a player's career.}}
    \label{fig:importance}
\end{figure}

Figure \ref{fig:3d-plot} visualizes the prediction function learned by the random forest, plotting predicted QBR against \revision{career passing attempts per season and career rushing yards per season. We observe that the highest predictions tend to occur for quarterbacks with relatively high passing volume combined with meaningful rushing production, reflecting the modern dual-threat profile that translates well to the NFL.} The extreme outlier by passing yards per season is Bailey Zappe, who played only one FBS season at Western Kentucky in Conference USA and was selected by the New England Patriots in the fourth round of the 2022 NFL Draft. The predictions of the random forest model are conservative, with extremely few quarterbacks predicted to exceed a QBR of 40. This speaks to the large number of college quarterbacks who achieve zero QBR in the NFL and the difficulty of distinguishing between prospects based on college performance statistics alone.
...
Table \ref{tab:top-ten} shows the QBR predictions for the top 10 predicted quarterbacks in the 2025 draft. \revision{Using both the classification and regression models, predictions are calculated as  
\begin{equation}
...
Recall that the similarity scores reported in this table are interpretable as the weight of each historical prospect in the predictions in Table \ref{tab:top-ten}, \revision{which combine both the estimated probability of a positive QBR (classification model) and the conditional expected QBR given a positive outcome (regression model).} The highest similarity score for any of these prospects is only \revision{3.7\%}. This is a helpful reminder that quarterback career outcomes are noisy and speaks to the fallacy of putting significant stock in a single player comparison. The random forest model learns to spread out the weight across historical players, rather than concentrating a lot of weight in the single most similar player. With access to more granular athleticism data, the weight might concentrate more in fewer comps.

\revision{For each of the prospects, we are able to calculate their effective sample size (ESS) to capture the number of training players from a random sample to obtain a prediction \citep{mccaffrey_propensity_2004}, with the formula: $$\text{ESS}=\frac{\sum_{i=1}^n(w_{i})^2}{\sum_{i=1}^{n}w_i^2}.$$}

\revision{Additionally, modifying the model's hyperparameters can result in sparser and more concentrated weights. Figure \ref{fig:ESS-vs-hyperparameter-values} is a box plot showing the relationship between the effective sample size and three random forest hyperparameters. We can see that reducing the minimum node size results in sparser weights, while reducing max depth results in more concentrated weights. The relationship with mtry is not as strong as the other two hyperparameters, but an mtry value of 1 appears to result in more concentrated weights. However, as mtry increases past 1, there is a plateau.
}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ESS_vs_hyperparameter_values.pdf}
    \caption{\it \revision{Visualization of the relationship between effective sample size and the max depth, min node size, and mtry hyperparameters in both the classification and regression models. In both models, lower max depth values were associated with higher ESS values, and lower min node size values were associated with lower ESS values.
    }}
    \label{fig:ESS-vs-hyperparameter-values}
\end{figure}

\revision{Figure \ref{fig:comp-pct-plot} shows the number of comparable players required to account for a given share of the prediction weight and states the ESS value for each player in the caption.}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{comp_pct_plot.pdf}
    \caption{\it \revision{Cumulative percent of prediction explained by the top comparable players. The corresponding ESS values for each of these players are 81.5 for Cameron Ward, 96.0 for Shedeur Sanders, 96.0 for Jaxson Dart, and 75.60 for Tyler Shough.
    }}
