...
The two-stage random forest model explained 29.7\% of the variance of $\text{QBR}_\text{reg}$ in the held-out test set. For comparison, the two-stage GLM explained 28.3\% of the test variance, and the two-stage $k$-NN model explained 25.1\%. Figure \ref{fig:3d-plot} visualizes the prediction function learned by the random forest, plotting predicted $\text{QBR}_\text{reg}$ against final-season strength of schedule and career passing yards per attempt. The highest predictions tend to occur for quarterbacks who achieved strong performance against strong opposition. The predictions of the random forest model are conservative, with extremely few quarterbacks predicted to exceed a $\text{QBR}_\text{reg}$ of 40.
...
\begin{figure}
    ...
    \caption{
      \it Feature importance for first-stage (classification) and second-stage (regression) random forests. The first-stage model predicts the probability of nonzero $\text{QBR}_\text{reg}$ in the NFL, and the second-stage model predicts expected NFL $\text{QBR}_\text{reg}$ conditional on it being nonzero.
    }
