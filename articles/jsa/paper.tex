...
\revision{The two stage random forest model explained 29.7\% of the variance of \revision{$\text{QBR}_\text{reg}$} in the held-out test set. For comparison, the two-stage GLM explained 28.3\% of the test variance, and the two-stage KNN model explained 25.1\% of the test variance.} Figure \ref{fig:3d-plot} visualizes the prediction function learned by the random forest, plotting predicted \revision{$\text{QBR}_\text{reg}$} against career passing attempts per season and career rushing yards per season. The highest predictions tend to occur for quarterbacks with relatively high passing volume combined with meaningful rushing production, reflecting the modern dual-threat profile that translates well to the NFL. The predictions of the random forest model are conservative, with extremely few quarterbacks predicted to exceed a \revision{$\text{QBR}_\text{reg}$} of 40. This speaks to the large number of college quarterbacks who achieve zero \revision{$\text{QBR}_\text{reg}$} in the NFL and the difficulty of distinguishing between prospects based on college performance statistics alone.
...
\revision{To illustrate our case-based approach to interpreting random forest predictions, we focus on the 2025 quarterback draft class.} For background, Table \ref{tab:top-ten} shows the \revision{$\text{QBR}_\text{reg}$} predictions for the top 10 predicted quarterbacks in the 2025 draft. Cam Ward is predicted to have the second highest \revision{$\text{QBR}_\text{reg}$}, which is a slight mismatch from the common consensus that he was the best quarterback in this draft class and the fact that he was taken first overall in the draft. Dillon Gabriel is projected to have the highest \revision{$\text{QBR}_\text{reg}$}, however, this can likely be attributed to the significant difference in final season strength of schedule (5.83 for Gabriel and 1.58 for Ward). Shedeur Sanders was projected to have the third-highest \revision{$\text{QBR}_\text{reg}$}, respectively. While Dillon Gabriel was not previously expected to be drafted as high as Shedeur Sanders, he had a very consistent college career and finished the highest of any quarterback in Heisman voting in 2024. Interestingly, Gabriel (third round) and Sanders (fifth round) were both selected by the Cleveland Browns after Sanders slid surprisingly deep into the draft, as we discuss later. Jaxson Dart's draft stock increased substantially after the NFL Draft Combine and his pro day. Notably, Tyler Shough is not in the top 10 predicted quarterbacks, even though he was the third quarterback taken in the draft after Ward and Dart. He was the 40th overall pick by the New Orleans Saints.

\begin{table}
  \centering
  \input{top_ten.tex}
  \caption{\it Top ten career-average \revision{$\text{QBR}_\text{reg}$} predictions for quarterback prospects in the 2025 draft class. This includes draft-eligible players who played at an NCAA Division I FBS school during the 2024 season.}
  \label{tab:top-ten}
\end{table}

While the predictions are interesting by themselves, the focus of the present work is interpreting how we arrive at them. Table \ref{tab:side-by-side-similarity} reports the ten most similar historical draft prospects for four quarterback prospects: Cam Ward, Shedeur Sanders, Jaxson Dart, and Tyler Shough\revision{, whose predicted $\text{QBR}_\text{reg}$ was only 2.6 despite being drafted in the second round}. These similarity scores result from the regression model, so the comps are players that had a \revision{$\text{QBR}_\text{reg}$} value above zero. Recall that the similarity scores reported in this table are interpretable as the weight of each historical prospect in the predictions in Table \ref{tab:top-ten}, which combine both the estimated probability of a positive \revision{$\text{QBR}_\text{reg}$} (classification model) and the conditional expected \revision{$\text{QBR}_\text{reg}$} given a positive outcome (regression model). The highest similarity score for any of these prospects is only 4.6\%. This is a helpful reminder that quarterback career outcomes are noisy and speaks to the fallacy of putting significant stock in a single player comparison. The random forest model learns to spread out the weight across historical players, rather than concentrating a lot of weight in the single most similar player. With access to more granular athleticism data, the weight might concentrate more in fewer comps.
...
In the application to predicting career outcomes for quarterback prospects in the NFL draft, the random forest's predictions explain \revision{29.7\% of the variance in NFL career outcomes. Evaluating quarterback prospects is notoriously difficult, which is why teams get it wrong all the time, even with access to higher-quality proprietary data. The low predictability makes interpretability of predictions even more important because executives cannot rely on a single opaque number. The predictions still carry significant signal, and decision-makers need to understand the source of that signal to effectively blend it with other sources of information.}
