...
\runninghead{Models in the War Room}

\title{Models in the War Room: Translating random forest predictions into player comps}
...
\section{Application: NFL Draft Prospects}
...
To validate the performance of the two-stage random forest model, we benchmark its out-of-sample performance in predicting $\text{QBR}_\text{reg}$ against two simple alternatives: a two-stage generalized linear model (GLM) and a two-stage $k$-NN algorithm, using the same features as the random forest models. In the two-stage linear regression model, we first estimate a logistic regression model for the probability of nonzero $\text{QBR}_\text{reg}$ and then estimate a second linear regression model for expected $\text{QBR}_\text{reg}$ conditional on it being nonzero. The $k$-NN model uses the same two stages, with $k = 47$ neighbors in the first stage and $k = 39$ neighbors in the second stage, tuned via the same 17-fold cross-validation procedure as the random forest model. We evaluate all models on the held-out test data of quarterbacks who completed their college careers in 2004, 2008, 2012, 2016, or 2020, which were not used for hyperparameter tuning.

\revision{
  In the \nameref{sec:results} section, we present our case-based explanation of predictions based on player comps alongside a feature-based explanation of predictions based on SHAP values. We calculated SHAP values using the TreeSHAP algorithm \citep{lundberg_local_2020}, as implemented in the R package treeshap \citep{mayer_treeshap_2026}. We created data visualizations based on these SHAP values using the R package shapviz \citep{mayer_shapviz_2025}.
}

\section{Results}
\label{sec:results}

The two-stage random forest model explained \revision{30.2\%} of the variance of $\text{QBR}_\text{reg}$ in the held-out test set. For comparison, the two-stage GLM explained 28.3\% of the test variance, and the two-stage $k$-NN model explained 25.1\%. \revision{Figure \ref{fig:shap-importance} shows the standard SHAP summary plot, providing an overview of the prediction function learned by the two-stage random forest model. In the first stage, the most important features were final-season strength of schedule, career passing yards per season, and final-season passer rating. In the second stage, the most important features were career games per season, career rushing touchdowns per season, and final-season passer rating. For most of these features, their relationship with the prediction was straightforward: Higher values contributed to higher predictions. However, the interpretation of final-season passer rating is more complicated because values near the middle of the distribution could have positive or negative contributions to both predictions, depending on interactions with other features in the models.}

\begin{figure}
    \centering
    \includegraphics[width=0.44\linewidth]{shap_classification.pdf}
    \includegraphics[width=0.55\linewidth]{shap_regression.pdf}
    \caption{
      \revision{
        \it SHAP summary plot for first-stage regression model (left) and second-stage classification model (right). Each point represents a player-feature, with the color representing the feature's value and the x-axis representing the feature's contribution to the player's prediction (the SHAP value). Features are sorted from top to bottom in order of importance.
      }
    }
    \label{fig:shap-importance}
\end{figure}

  While the feature-based interpretation illustrated by Figure \ref{fig:shap-importance} provides interesting macro-level insights on the prediction function learned by the random forest, it does not speak the language of scouting. It does not explain how historical prospect outcomes contribute to current prospect predictions. To translate model predictions into the vernacular of scouting, we need the case-based interpretation of the individual player predictions.
...
\begin{table}
  ...
  \caption{\it Top ten $\text{QBR}_\text{reg}$ predictions for quarterback prospects in the 2025 draft class. Names in bold are included in Figure \ref{fig:prospect-plots}. Shough's prediction is outside the top ten, but the table includes him for comparison because he was the third quarterback selected in the draft, after Ward and Dart.}
  \label{tab:top-ten}
\end{table}

Figure \ref{fig:prospect-plots} provides a case-based visualization of how the random forest arrived at its predictions for Ward, Dart, Shough, and Gabriel. Each prospect's prediction was a weighted average of past prospects' outcomes, and the weighted histograms in Figure \ref{fig:prospect-plots} visualize that weighted mean. Ward had the highest concentration of comps between 55 and 65 $\text{QBR}_\text{reg}$, which is why his prediction was the highest of these four prospects. On the opposite end of the spectrum, Shough had the highest concentration of training players near zero $\text{QBR}_\text{reg}$, explaining why his prediction was by far the lowest of the four prospects.
...
\begin{figure}
    ...
    \caption{\it Top: Histogram of historical players' $\text{QBR}_\text{reg}$ weighted by similarity from the second-stage (regression) random forest, with predicted zero-inflation from the first-stage (classification) random forest. The vertical orange line annotates each prospect's predicted $\text{QBR}_\text{reg}$, which matches the mean of the weighted distribution. Bottom: Scatter plot showing individual similarity scores (from the second-stage model) and $\text{QBR}_\text{reg}$ for historical prospects.}
...
\begin{table}
  ...
  \caption{\it Top ten similarity scores from the two-stage random forest for Tyler Shough and Dillon Gabriel. The similarity score is the contribution made by each historical prospect to the weighted average which constitutes the reference prospect's prediction in the random forest model.}
  \label{tab:side-by-side-similarity}
\end{table}

\revision{
  For comparison, Figure \ref{fig:shap-waterfall} shows a feature-based explanation of Shough's and Gabriel's predictions, using SHAP values. For Shough, the most negative contributor to both predictions (classification and regression) was that he averaged fewer than 100 pass completions per season in his college career. Relatedly, his low career passing attempts (158) and yards (1,303) per season contributed negatively to his first-stage prediction, offsetting positive contributions from his final-season strength of schedule (4.36) and career passing yards per attempt (8.22). For Gabriel, the most positive contributor to both predictions was that he finished third in Heisman voting in his final college season. Additionally, his final-season passer rating (165) and career passing yards per season (3,120) help explain his high first-stage prediction, and his career rushing touchdowns per season (5.5) and final-season game total (14) help explain his high second-stage prediction. The feature-based explanations from SHAP values provide a different perspective and are not directly comparable with the case-based explanations from player comps.
}

\begin{figure}
  \includegraphics[width=0.49\textwidth]{shap_classification_shough.pdf}
  \includegraphics[width=0.49\textwidth]{shap_classification_gabriel.pdf}\\
  \includegraphics[width=0.49\textwidth]{shap_regression_shough.pdf}
  \includegraphics[width=0.49\textwidth]{shap_regression_gabriel.pdf}
  \caption{
    \revision{
      \it Waterfall plots of SHAP values explaining the predictions for Tyler Shough (left) and Dillon Gabriel (right) from the first-stage classification model (top) and second-stage regression model (bottom). The waterfall plot shows the contribution of each feature to the individual prediction, tracing the path from the average prediction E[f(x)] to the individual prediction f(x).
    }
  }
...
\label{sec:discussion}
  ...
  \revision{
    SHAP values (Figure \ref{fig:shap-waterfall}) provide a useful framework for explaining individual player predictions, but they are not grounded in the historical player outcomes that are central to traditional scouting discussions. In a debate about a prospect, the two sides may disagree about the importance of pass completions per season (Shough's weakness) or Heisman voting (Gabriel's strength). Framing the conversation around concrete outcomes for historical prospects narrows the gap between model and scouting process. At minimum, our approach provides an alternative lens for interpreting random forest predictions, serving as an extra tool for reconciling disagreements between quantitative modeling and scouting opinion.
  }

  The random forest prediction may not be better than the scouts' predictions, and this is precisely why interpretability is so important. The model explained \revision{30.2\%} of variance in the test set. Evaluating quarterback prospects is notoriously difficult, which is why teams often get it wrong, even with higher-quality proprietary data. The low predictability makes interpretability of predictions even more important because executives cannot rely on a single opaque number. The predictions still carry significant signal, and decision-makers need to understand the source of that signal to effectively synthesize it with other sources of information. The similarity scores extracted from the random forest lay bare its predictions in the vernacular of traditional scouting, facilitating that synthesis.

  The specific application of predicting NFL QBR for college quarterbacks is limited by the number of quarterback careers available for training and a low signal-to-noise ratio when using public college performance statistics to predict professional career outcomes. Additionally, the use of $\text{QBR}_\text{reg}$ as the outcome variable presents limitations due to zero-inflation and censoring, as we addressed more deeply in the \nameref{sec:data} section. Future work could seek a stronger measure of quarterback career outcomes and incorporate additional information from the NFL Draft Combine to explain more of the variance in the outcome. Teammate support, such as strength of receiving corps, may explain additional variance.

  However, the idea of extracting similarity scores from random forests to translate the predictions into the language of player comps in professional sports applies more broadly beyond this specific application. \revision{Our methodology represents a mathematical formalization of traditional player scouting, born from the connection between historical scouting comps and the prediction mechanics of random forests. By translating black-box predictions into the native language of scouting, our framework directly addresses the practical problem of model adoption in the front office, providing a way to bridge the gap between quantitative and qualitative evidence in the draft room.} We have published the R package treecomp, which allows researchers and practitioners to extract similarity scores from their own random forest models. Future work could extend this work to other tree-based regression and classification models, such as gradient boosting.
