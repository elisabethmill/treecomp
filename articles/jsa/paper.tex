...
\runninghead{\revision{Speaking the Language of Scouting}}

\title{\revision{Speaking the Language of Scouting: Translating random forest predictions into player comps}}
...
% Discuss how this paper uses Lin and Jeon
...
All data used for this study are publicly available from Sports Reference \citep{sports_reference_sports_2025}. Our dataset includes 803 quarterbacks who (1) played their final season at the NCAA Division I Football Bowl Subdivision (FBS) between 2001 and 2022 (inclusive); and (3) attempted more than 500 passes during their career. We found that only 8\% of NFL quarterbacks attempted fewer than 500 career passes in college, and these players do not have substantial statistical records for predicting their NFL careers. The data for each player-season include team, conference, strength of schedule, games played, passing statistics (attempts, completions, touchdowns, interceptions), rushing statistics (attempts, yards, touchdowns), and awards (All-America designation, Heisman voting).
...
While the predictions are interesting by themselves, the focus of the present work is interpreting how we arrive at them. Table \ref{tab:side-by-side-similarity} reports the ten most similar historical draft prospects for four of the top quarterback prospects: Cam Ward, Shedeur Sanders, Jaxson Dart, and Tyler Shough. These similarity scores result from the regression model, so the comps are players that have/had a QBR value above zero. Recall that the similarity scores reported in this table are interpretable as the weight of each historical prospect in the predictions in Table \ref{tab:top-ten}, which combine both the estimated probability of a positive QBR (classification model) and the conditional expected QBR given a positive outcome (regression model). The highest similarity score for any of these prospects is only 4.6\%. This is a helpful reminder that quarterback career outcomes are noisy and speaks to the fallacy of putting significant stock in a single player comparison. The random forest model learns to spread out the weight across historical players, rather than concentrating a lot of weight in the single most similar player. With access to more granular athleticism data, the weight might concentrate more in fewer comps.
...
\begin{figure}[H]
    ...
    \caption{\it Top: Histogram of historical players' regressed QBR weighted by similarity from the second-stage (regression) random forest, with predicted zero-inflation from the first-stage (classification) random forest. The vertical orange line annotates each prospect's predicted QBR, which matches the mean of the weighted distribution. Bottom: Scatter plot showing individual similarity scores (from the second-stage model) and regressed QBR for historical prospects.}
