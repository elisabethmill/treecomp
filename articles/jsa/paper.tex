...
\revision{The two stage random forest model explained 29.7\% of the variance of \revision{$\text{QBR}_\text{reg}$} in the held-out test set. For comparison, the two-stage GLM explained 28.3\% of the test variance, and the two-stage KNN model explained 25.1\%.} Figure \ref{fig:3d-plot} visualizes the prediction function learned by the random forest, plotting predicted \revision{$\text{QBR}_\text{reg}$} against \revision{final-season strength of schedule and career passing yards per attempt}. The highest predictions tend to occur for quarterbacks {who achieved strong performance against strong opposition.} The predictions of the random forest model are conservative, with extremely few quarterbacks predicted to exceed a \revision{$\text{QBR}_\text{reg}$} of 40.
...
\revision{
  To illustrate our case-based approach to interpreting random forest predictions, we focus on the 2025 quarterback draft class. Table \ref{tab:top-ten} shows the $\text{QBR}_\text{reg}$ predictions for the top 10 predicted quarterbacks in the 2025 draft. We highlight four players for interpretation. The first two quarterbacks selected, Cam Ward (first overall) and Jaxon Dart, were predicted to have the second- and fourth-highest $\text{QBR}_\text{reg}$, respectively. An interesting case is Shedeur Sanders, who had the third-highest projected $\text{QBR}_\text{reg}$ but slid deep into the draft because of his refusal to participate in the NFL Scouting Combine, along with personality concerns \citep{mckenna_what_2025}. A notable omission from Table \ref{tab:top-ten} is Tyler Shough, who was the third quarterback selected (after Ward and Dart) but is predicted by the two-stage random forest model for only 2.6 $\text{QBR}_\text{reg}$.
}
...
\begin{table}
  ...
  \caption{\it Top ten \revision{$\text{QBR}_\text{reg}$} predictions for quarterback prospects in the 2025 draft class. Shough's prediction is outside the top ten, but the table includes him for comparison because he was the third quarterback selected in the draft, after Ward and Dart.}
  \label{tab:top-ten}
\end{table}

\revision{Figure \ref{fig:prospect-plots} provides a case-based visualization of how the random forest arrives at its predictions for Ward, Sanders, Dart and Shough.} Each prospect's prediction is a weighted average of past prospects' outcomes, and the weighted histograms in Figure \ref{fig:prospect-plots} visualize that weighted mean. Ward has the highest concentration of comps between 55 and 65 \revision{$\text{QBR}_\text{reg}$}, which is why his prediction is the highest of these four prospects. On the opposite end of the spectrum, Shough has the highest concentration of training players near zero \revision{$\text{QBR}_\text{reg}$}, explaining why his prediction was by far the lowest of the four prospects.
